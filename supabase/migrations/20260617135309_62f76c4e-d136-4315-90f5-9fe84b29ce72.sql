...
DECLARE
  ...
  _new_count integer;
BEGIN
  SELECT i.id, i.status, i.app_cap AS cap, i.slots, i.application_count AS cnt, i.deadline
    INTO _internship
  ...
  -- P0-1: Reject applications past the deadline at the DB layer too.
  IF _internship.deadline IS NOT NULL AND _internship.deadline < now() THEN
    RETURN QUERY SELECT false, 'DEADLINE_PASSED', 'Application deadline has passed', _internship.cnt, _internship.cap; RETURN;
  END IF;
  ...
  UPDATE public.internships AS i2
     SET application_count = COALESCE(i2.application_count, 0) + 1
   WHERE i2.id = p_internship_id
  RETURNING i2.application_count INTO _new_count;

  IF _internship.cap IS NOT NULL AND _new_count >= _internship.cap THEN
    UPDATE public.internships AS i3 SET status = 'closed' WHERE i3.id = p_internship_id;
  END IF;
